...
pub fn get_byte_rows_for_snake(mouse: &Point, snake: &Snake, display_index: usize) -> [u8; 8] {
    let mut byte_rows = get_byte_rows_for_point(&mouse, display_index);
...
pub fn get_byte_rows_for_point(point: &Point, display_index: usize) -> [u8; 8] {
    ...
    let actual_display_index = point.x / crate::DISPLAY_SIZE;
...
impl Display {
    pub fn write(&mut self, snake: &Snake, mouse: &Point) -> () {
        for i in 0..crate::NUMBER_DISPLAYS {
            self.max7219
                .write_raw(i, &get_byte_rows_for_snake(&mouse, &snake, i))
